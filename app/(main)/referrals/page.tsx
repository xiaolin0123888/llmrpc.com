...
export default async function ReferralsPage() {
  ...
  return (
    <div style={{ maxWidth: 750, margin: '0 auto', padding: '2rem' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #0f172a 100%)', borderRadius: 16, padding: '2rem 2rem 1.5rem', marginBottom: '2rem', border: '1px solid #2563eb33' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
          <div>
            ...
            <p style={{ fontSize: '1.4rem', fontWeight: 700, color: '#fff', fontFamily: 'monospace' }}>{user?.referral_code?.toUpperCase() ?? 'N/A'}</p>
          </div>
          <div>
            ...
            <p style={{ fontSize: '1.4rem', fontWeight: 700, color: '#fff' }}>{user?.referral_count ?? 0}</p>
          </div>
          <div>
            ...
            <p style={{ fontSize: '1.4rem', fontWeight: 700, color: '#22c55e' }}>{(user?.referral_count ?? 0) * 500000 > 0 ? ((user?.referral_count ?? 0) * 500000).toLocaleString() : '0'}</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
          {[
            ...
            { step: '3', icon: '💰', title: 'Both Get Rewards', desc: 'They get 1M free credits. You get 500K for each invite.' },
          ].map(f => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
        {[
          { icon: '🎁', title: 'You Get 500K', desc: '500,000 credits credited instantly when they verify email', highlight: true },
