...
function isNetworkError(err: unknown): boolean {
  return err instanceof TypeError && (err.message.includes("fetch") || err.message.includes("network") || err.message.includes("ENOTFOUND") || err.message.includes("ECONNREFUSED") || err.message.includes("ETIMEDOUT") || err.message.includes("certificate") || err.message.includes("TLS") || err.cause !== undefined)
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isNetworkError(err)) {
      console.error("[paypal capture-order] Network error:", err.message)
